...
function get_lang_menu_list()
{
    ...
    echo '<p class="col-label">' . __('Language', 'anomeo') . '</p>';
...
{
    if (WC()->cart->is_empty()) {
        return '<a class="desktop-menu-btn icon icon-cart" id="cart-popup-toggle" href="">' . __('', 'anomeo') . '</a>';
    } else {
        return '<a class="desktop-menu-btn icon icon-cart" id="cart-popup-toggle" href="">' . __('', 'anomeo') . WC()->cart->cart_contents_count . '</a>';
    }
}
...
{ ?>
    <div class="site-cookies-banner">
        <p><? _e('We care about your privacy. We use cookies and similar technologies to provide the best experience on our website.', 'anomeo'); ?> <a href="/privacy-policy/"><? _e('Privacy Policy', 'anomeo'); ?></a></p>

        <button class="close-btn"><? _e('Ok', 'anomeo'); ?></button>
    </div>
<?
}
...
function product_back_to_category_link()
{
    ...
    if (count($filtered_cats) > 0) {
        ...
        return '<a href="' . get_home_url() . '">' . __('Home', 'anomeo') . '</a>';
    }
}
